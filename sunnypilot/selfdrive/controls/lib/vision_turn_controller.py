...
_ENTERING_PRED_LAT_ACC_TH = 0.9  # Predicted Lat Acc threshold to trigger entering turn state.
_ABORT_ENTERING_PRED_LAT_ACC_TH = 0.6  # Predicted Lat Acc threshold to abort entering state if speed drops.

_TURNING_LAT_ACC_TH = 1.1  # Lat Acc threshold to trigger turning state.

_LEAVING_LAT_ACC_TH = 0.9  # Lat Acc threshold to trigger leaving turn state.
_FINISH_LAT_ACC_TH = 0.7  # Lat Acc threshold to trigger the end of the turn cycle.
...
_TURNING_ACC_V = [0.2, 0., -0.6]  # acc value
_TURNING_ACC_BP = [1.0, 1.8, 3.]  # absolute value of current lat acc
