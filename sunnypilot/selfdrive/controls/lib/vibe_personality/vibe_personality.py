...
MAX_ACCEL_PROFILES = {
    AccelPersonality.eco:       [1.7,  1.30,  1.10, 0.70, .500, .292, .13, .09],  # eco
...
MIN_ACCEL_PROFILES = {
    LongPersonality.relaxed:    [-.2, -.5, -.8,  -1.15, -1.4],  # gentler braking
    LongPersonality.standard:   [-.2, -.5, -.8,  -1.15, -1.4],  # normal braking
    LongPersonality.aggressive: [-.2, -.5, -.8,  -1.15, -1.4],  # more aggressive braking
}
MIN_ACCEL_BREAKPOINTS =         [0.,   1.5,    4.,   9.,  19.]


def get_T_FOLLOW_vibe(personality):
    """Get base T_FOLLOW value for each personality"""
    return {
        LongPersonality.relaxed: 2.05,
        LongPersonality.standard: 1.85,
        LongPersonality.aggressive: 1.65
    }[personality]


def get_dynamic_personality(v_ego, personality):
    """Adjust T_FOLLOW based on vehicle speed"""
    scale_factor = np.interp(v_ego, [0, 28], [1.0, 1.0])
