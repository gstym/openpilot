...
MIN_ACCEL_PROFILES = {
    LongPersonality.relaxed:    [-.0030, -.0030, -.8,  -1.0, -1.2],  # gentler braking
    LongPersonality.standard:   [-.0030, -.0030, -.8,  -1.0, -1.2],  # normal braking
    LongPersonality.aggressive: [-.0030, -.0030, -.8,  -1.0, -1.2],  # more aggressive braking
}
...
def get_T_FOLLOW_vibe(personality):
    ...
    return {
        LongPersonality.relaxed: 1.80,
        LongPersonality.standard: 1.60,
        LongPersonality.aggressive: 1.60
    }[personality]


def get_dynamic_personality(v_ego, personality):
    """Adjust T_FOLLOW based on vehicle speed"""
    scale_factor = np.interp(v_ego, [0, 28], [0.75, 1.0])
